...
gomp_init_offload_manager ( )
{
    ...
    if(nb_offload_funcs) {
        //FIXME mailbox_write(TO_RUNTIME | nb_offload_funcs);
...
#endif
            //FIXME mailbox_write((uint32_t) offload_func_table[i]);
        }
#ifdef OFFLOAD_MANAGER_VERBOSE
        printf ("\n");
#endif
    ...
    if(nb_offload_vars)
    {
        //FIXME mailbox_write(TO_RUNTIME | 2*nb_offload_vars);
#ifdef OFFLOAD_MANAGER_VERBOSE
...
#endif
            //FIXME mailbox_write((uint32_t) offload_var_table[2*i]);
            //FIXME mailbox_write((uint32_t) offload_var_table[2*i+1]);
        }
#ifdef OFFLOAD_MANAGER_VERBOSE
        printf ("\n");
#endif
